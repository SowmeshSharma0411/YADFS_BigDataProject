...
commands = ["upload_file", "get_file", "get_info", "exit", "create_directory", "move_file", "move_folder",
            "re_replicate", "delete_file", "get_directory", "list_directory", "datanode_status", "delete_folder", "copy_file"]
command_completer = WordCompleter(commands)

# NameNode API endpoint
namenode_url = "http://localhost:8090"
...
def handle_create_directory(directory_path):
    response = requests.post(
        f"{namenode_url}/create_directory", data={'directory_path': directory_path})
    print(response.text)


def handle_move_file(original_path, destination_path, file_name):
    response = requests.post(f"{namenode_url}/move_file", data={"original_path": original_path,
                             "destination_path": destination_path, "file_name": file_name})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)


def handle_move_folder(original_path, destination_path, folder_name):
    response = requests.post(f"{namenode_url}/move_folder", data={'original_path': original_path,
                             'destination_path': destination_path, 'folder_name': folder_name})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)


def handle_re_replicate(file_name, directory_path):
    response = requests.post(f"{namenode_url}/re_replicate",
                             data={'file_name': file_name, "directory_path": directory_path})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)
...
def handle_delete_file(file_name, directory_path):
    response = requests.post(f"{namenode_url}/delete_file",
                             data={'file_name': file_name, 'directory_path': directory_path})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)
...
def handle_list_directory(directory_path):
    response = requests.get(
        f"{namenode_url}/list_directory", data={'directory_path': directory_path})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)


def handle_delete_folder(folder_name, directory_path):
    response = requests.post(f"{namenode_url}/delete_folder",
                             data={'folder_name': folder_name, 'directory_path': directory_path})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)


def handle_copy_file(original_path, destination_path, file_name):
    response = requests.post(f"{namenode_url}/copy_file", data={'original_path': original_path,
                             'destination_path': destination_path, 'file_name': file_name})
    indented_json = json.dumps(response.json(), indent=2)
    print(indented_json)
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            ...
            directory_path = input(
                "Enter the directory path (optional, press Enter to use '/'): ")
